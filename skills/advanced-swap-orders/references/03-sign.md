...
3. Submit with `--prepared <prepared.json|->` and `--signature <0x...|json>`, `--signature-file <sig.txt|sig.json|->`, or `--r <0x...> --s <0x...> --v <0x...>`. The helper accepts a full 65-byte signature, a JSON string, or JSON/object `r/s/v`.
