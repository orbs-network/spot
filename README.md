...
Spot supports a chain when all six core contracts and at least one shared (`*`) or chain-specific adapter are deployed. See [`config.json`](./config.json) for explicit chain entries and runtime addresses, and [`skill/SKILL.md`](./skill/SKILL.md#supported-chains) for the supported chain list. Mantle (`5000`) and Manta Pacific (`169`) qualify through deployed shared adapters despite their empty chain-specific adapter and DEX maps.
