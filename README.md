...
With `NOTION_API_KEY`, tests read every page of the integrations board and check SPOT rows against current configuration and observations. They detect missing, duplicate, and stale integrations; incorrect chain sets or solver labels; and drift in contract, oracle, taker, and relay readiness. For each component, full chain coverage expects `Done`, partial coverage `In progress`, and no coverage `Not started`. Taker comparisons cover observed taker operation, not the other systems bundled into that column's title. A source outage or chains outside a scoped run leave the corresponding status unchecked rather than inventing a replacement. TWAP/LH rows and other team readiness fields are excluded. Tests never update Notion. Board drift or an unavailable Notion board produces warnings without failing the run; runtime dependency failures still fail. `npm run test:runtime` checks this logic offline.
