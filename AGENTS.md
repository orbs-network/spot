...
4. For each default order, size `input.maxAmount` to about `$15` of that leg's input token to stay above the relay's `$10` order minimum, use exactly 2 equal chunks so `input.amount = input.maxAmount / 2` is about `$7.50` per chunk, and set `epoch = 60`.
