...
4. For each default order, size `input.maxAmount` to about `$30` of that leg's input token, use exactly 2 equal chunks so `input.amount = input.maxAmount / 2` is about `$15` per chunk and stays above the relay's `$10` minimum, and set `epoch = 60`.
