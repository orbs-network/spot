...
Use the `TWAP/Spot Integrations` database (`***REMOVED***`). Clear only the `Contracts` property for Mantle, Manta, and Moonbeam. Preserve the rows and every other property, including `Oracle`.
